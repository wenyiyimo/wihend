...
const formattor = [
	...
	// code 样式
	['code', "font-size:14px; line-height:18px; background:#F6F7F8; padding-top: 4px; padding-bottom: 4px; padding-left: 4px; padding-right: 4px;"],
	// img 样式
	['img', "width:750rpx;height:1400rpx; margin-top: 4px; margin-bottom: 4px;"],
	// blockquote
	['blockquote',
		...
		for (let i = 0; i < formattor.length; i++) {
			...
			html = html.replace(reg, function(word) {
				...
				if (word.indexOf('style=') != -1) {
					...
					return word.replace(regIn, '<' + formattor[i][0] + '$1style="$2 ' + formattor[i][1] + '"$3$4>')
				} else {
