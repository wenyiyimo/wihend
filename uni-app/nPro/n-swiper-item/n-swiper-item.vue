<template>
	<view ref="n-swiper-item" :class="['n-position-absolute', 'n-bg-'+bgType, 'n-border-'+border, 'n-radius-'+radius, 'n-flex-'+flex, 'n-align-'+align, 'n-justify-'+justify]" :style="mrStyle+boxStyle">
		<slot></slot>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			mrStyle() {
				let style = ''
				let left = this.nSwiper.leftPx + this.nSwiper.spacePx * this.index + this.nSwiper.itemWidthPx * this.index
				if (this.nSwiper.center) {
					left = (this.nSwiper.widthPx - this.nSwiper.itemWidthPx)*0.5 + this.nSwiper.spacePx * this.index + this.nSwiper.itemWidthPx * this.index
				}
				style += `left:${left}px;width:${this.nSwiper.itemWidthPx}px;;height:${this.nSwiper.itemHeightPx}px;`
				return style
			}
		},
		data() {
		...
		methods: {
			getExp(current, factor=1) {
				const baseWidth = this.nSwiper.baseWidthPx
				let left = - current * baseWidth
				const index = this.index
				const length = this.nSwiper.children.length
				if (this.nSwiper.circular) {
					left = this.toGetLeft(baseWidth, current, index, true)
					if (left === '') {
						return `0+${5*baseWidth}`
					}
				}
				// 最大允许拉开
				const maxX = this.nSwiper.maxX
				// 最外层必须加一个()，否则异常
				const x = `(abs(x*${factor}) >= ${maxX} ? (x>0?${maxX}:(${-1*maxX})) : (x*${factor}))`
				// console.log(x)
				if (this.nSwiper.linear) {
					return `${x}+${left}`
				}
				// 利用正弦函数来做动画曲线
				return `abs(sin(${3.14*0.5/maxX}*${x}))*${x}+${left}`
			},
			getScaleExp(current, factor=1) {
				const sameExp = `${this.nSwiper.scale}+0`
				const baseWidth = this.nSwiper.baseWidthPx
				const minusExp = `max(1 - abs(x*${factor}/${baseWidth}), ${this.nSwiper.scale})`
				const plusExp = `min(1, ${this.nSwiper.scale}+abs(x*${factor}/${baseWidth}))`
				const circle = this.nSwiper.circular
				const index = this.index
				const length = this.nSwiper.children.length
				let exp = sameExp
				if (current === index) {
			...
			toAdjust(x, time, init) {
				...
				let iScale = scale
				const percent = this.nSwiper.percent
				...
				if (circle) {
					...
					//
					if (init) {
						this.toInitCircle(baseWidth, current, index, iScale)
						return
					}
					// 先平移，移动之后再补充内容
					left = this.toGetLeft(baseWidth, current, index)
					if (current != toCurrent) {
						left += x >= 0 ? baseWidth : -baseWidth
					}
				}
				animation.transition(this.$refs['n-swiper-item'], {
					styles: {
					...
					if (needChange && circle) {
						...
						that.toSupplyCircle(baseWidth, current, toCurrent, length)
					}
					if (needChange && toCurrent === index) {
						that.nSwiper.$emit("change", toCurrent)
					}
				})
			},
			toPosition(current, toCurrent, silence) {
				const that = this
				const length = this.nSwiper.children.length
				const index = this.index
				const duration = silence ? 0 : this.nSwiper.duration
				const baseWidth = this.nSwiper.baseWidthPx
				const scale = this.nSwiper.scale
				let iScale = scale
				if (toCurrent === index) {
			...
			toInitCircle(baseWidth, toCurrent, index, iScale) {
				const duration = this.nSwiper.duration
				const left = this.toGetLeft(baseWidth, toCurrent, index)
				let opacity = 0
				if (toCurrent === index) {
					opacity = 1
				}
				...
				animation.transition(this.$refs['n-swiper-item'], {
					...
					animation.transition(that.$refs['n-swiper-item'], {
						styles: {
							transform: `translateX(${left}px) scale(${iScale})`,
							opacity: opacity
						},
				...
				if (toLeft) {
					...
					if (this.index === popIdx) {
						animation.transition(this.$refs['n-swiper-item'], {
							...
							that.toOpacityShow(5*baseWidth, iScale)
						})
					}
					if (this.index === pushIdx) {
						animation.transition(this.$refs['n-swiper-item'], {
							...
							that.toOpacityShow(-1*baseWidth*(pushIdx-2), iScale)
						})
					}
					return
				}
			...
			toGetLeft(baseWidth, toCurrent, index, isExp) {
				const length = this.nSwiper.children.length
				let left = baseWidth * toCurrent * -1
				// circle的话，至少准备5个数据
				// 不足5个数据请自行重复拼凑，不会检验
