  ...
  <div style="margin-top: 200px">
    <a-form v-show="cart.length" :style="{width:'600px'}" >
      <a-form-item v-for="(item,index) in cart" :key="index" :label="item.name+':'+item.price+'元'">
          <a-input-number @change="countChange(index,$event)" :min="0" :max="item.stock" :style="{width:'320px'}" placeholder="Please Enter" :model-value="item.count" :default-value="0" mode="button" class="input-demo" />
      </a-form-item>
      <a-form-item field="originalPrice" label="原价">
          <a-input-number :style="{width:'320px'}" :model-value="realTotalPrice" placeholder="Please Enter" :default-value="0" class="input-demo" disabled/>
      </a-form-item>
      <a-form-item field="totalPrice" label="总价">
          <a-input-number :style="{width:'320px'}" :model-value="totalPrice" placeholder="Please Enter" :default-value="0" class="input-demo" disabled/>
      </a-form-item>
    </a-form>
